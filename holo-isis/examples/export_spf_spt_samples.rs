...
use holo_isis::spf_profiling::{collect_scale_samples, write_samples_csv, write_samples_json, SPF_METHOD_REAL};

fn main() {
    let out = env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| PathBuf::from("target/yqh162-spf-spt"));
    ...
    for s in &samples {
        acc.entry((s.n_nodes, s.mode.clone())).or_default().push(s.duration_us);
    }
    println!("spf_method={}", SPF_METHOD_REAL);
    println!("wrote {} samples -> {} {}", samples.len(), csv.display(), json.display());
    for ((n, mode), xs) in acc {
        let mean = xs.iter().sum::<f64>() / xs.len() as f64;
        let min = xs.iter().cloned().fold(f64::INFINITY, f64::min);
        let max = xs.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
        let verts = samples.iter().find(|s| s.n_nodes==n && s.mode==mode).map(|s| s.spt_vertices).unwrap();
        println!("n={n} mode={mode} verts={verts} mean_us={mean:.3} min_us={min:.3} max_us={max:.3} reps={}", xs.len());
    }
}
